        ...
        if (patternAndValue != null) {
            ...
            for (int i = 0; i < patternString.length(); i++) {
                ...
                switch (patternSymbol) {
                    case "+":
                        valueIterator++;
                        if (isPatternNotCorrect(patternPlusAZ, value, valueIterator)) {
                            return false;
                        }
                        break;
                    case "$":
                        valueIterator++;
                        if (isPatternNotCorrect(patternDollar09, value, valueIterator)) {
                            return false;
                        }
                        break;
                    case "*":
                        if (isPatternRepeatsByAsterisk(patternSymbol, patternString, i)) {
                            valueIterator += 3;
                            if (isPatternNotCorrect(patternAloneAsterisk, value, valueIterator)) {
                                return false;
                            }
                            ...
                            if (isPatternNotCorrect(patternAsterisk, value, valueIterator)) {
                                return false;
                            }
                        }
                        break;
    ...
    private static String[] validateData(String str) {
        ...
        try {
            if (patternString.contains(" ") || patternAndValue.length > 2) {
                throw new IllegalArgumentException("wzorzec rozpoczynajacy się " + patternString + " nie powinien zawierać więcej niż jedna spacja");
            } else if (checkCurlyBrackets(patternString)) {
                throw new IllegalArgumentException("wykryto problem związany z nawiasami {} we wzorcu " + patternString);
            } else if (!compareLength(patternString, value)) {
                throw new IllegalArgumentException("wykryto problem z oczekiwaną ilością znaków dla " + patternString + " oraz " + value);
            }
